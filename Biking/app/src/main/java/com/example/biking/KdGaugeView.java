...
public class KdGaugeView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mMinute = a.getInt(R.styleable.kdgaugeview_time_minute, 0);
        mSecond = a.getInt(R.styleable.kdgaugeview_time_minute, 0);
        ...
        mDistance= a.getFloat(R.styleable.kdgaugeview_distance, 0);
        mSafeSpeedLimit = a.getFloat(R.styleable.kdgaugeview_speed_limit, 60);
        mode = a.getString(R.styleable.kdgaugeview_mode);
        if (TextUtils.isEmpty(mode)) mode = "time";
...
//            canvas.drawArc(progressBarRect, 0, alertSpeedAngle, false, mSpeedProgressPaint);
    ...
    public void drawEnergy(Canvas canvas) {
        ...
        canvas.drawText("" + (int) mEnergy + " Cal", mCircleCenterX, mCircleCenterY - ((mSpeedTextPaint.descent() + mSpeedTextPaint.ascent()) / 2), mSpeedTextPaint);
        float x = mCircleCenterX;
        float y = (float) (mCircleCenterY + mDotedCircleRadius * Math.sin(Math.toRadians(-225)));
    ...
    public void drawClock(Canvas canvas) {
        ...
        //Text is always draw around the x coordinate and y coordinate is base line, so we have to shift the base line
        //in order to draw the text, consider the supplied point as center.
        if (mMinute < 10 & mSecond < 10) {
            canvas.drawText("" + "0" + mMinute + ":" +"0" + mSecond, mCircleCenterX, mCircleCenterY - ((mSpeedTextPaint.descent() + mSpeedTextPaint.ascent()) / 2), mSpeedTextPaint);
        } else if((mMinute < 10) & (mSecond >= 10)) {
            canvas.drawText("" +"0" + mMinute + ":" + mSecond, mCircleCenterX, mCircleCenterY - ((mSpeedTextPaint.descent() + mSpeedTextPaint.ascent()) / 2), mSpeedTextPaint);
        } else if((mMinute >= 10) & (mSecond < 10)) {
            canvas.drawText("" + mMinute + ":" +"0" + mSecond, mCircleCenterX, mCircleCenterY - ((mSpeedTextPaint.descent() + mSpeedTextPaint.ascent()) / 2), mSpeedTextPaint);
        } else {
            canvas.drawText("" + mMinute + ":" + mSecond, mCircleCenterX, mCircleCenterY - ((mSpeedTextPaint.descent() + mSpeedTextPaint.ascent()) / 2), mSpeedTextPaint);
        }
